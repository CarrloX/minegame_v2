...
export interface RaycastResult {
  ...
  distance: number;
  point?: THREE.Vector3; // Exact intersection point
  face?: {
    normal: THREE.Vector3;
  };
}
...
export class Player {
  ...
  public raycast(forceUpdate: boolean = false): RaycastResult | null {
    ...
    if (
      ...
      now - this.lastRaycastTime < this.RAYCAST_MAX_AGE
    ) {
    ...
    const nextX =
      stepX > 0 ? Math.floor(origin.x) + 1 : Math.ceil(origin.x - 1);
    const nextY =
      stepY > 0 ? Math.floor(origin.y) + 1 : Math.ceil(origin.y - 1);
    const nextZ =
      stepZ > 0 ? Math.floor(origin.z) + 1 : Math.ceil(origin.z - 1);
    ...
    while (t <= maxDistance && steps < maxSteps) {
      ...
      if (block !== undefined && block !== BlockType.AIR) {
        ...
        if (lastAxis === 0) {
          ...
          // Falls back to direction-based normal if immediate hit (inside block?)
          // or just a sane default.
          // Since we start t=0, if we are inside a block immediately, lastAxis is -1.
          // Usually we want to push out against the ray direction.
